...
def evaluate_accuracy(
    ...
    if total_ml == 0 and total_regex == 0:
        precision = recall = f1 = agreement_rate = 1.0
    else:
        precision = overlap_count / max(total_ml, 1)
        recall = overlap_count / max(total_regex, 1)
        f1 = 0.0 if precision + recall == 0 else 2 * precision * recall / (precision + recall)
        agreement_rate = overlap_count / max(union_size, 1)
