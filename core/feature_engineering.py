...
from .mitre import LOGIN_PATHS, SQL_KEYWORD_RE, XSS_INDICATORS, ATTACK_PATHS
...
def extract_per_request_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    full_url = pd.Series(
        [build_full_url(path_value, query_value) for path_value, query_value in zip(path, query)],
