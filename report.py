...
def plot_ml_vs_regex_comparison(accuracy_metrics: Optional[dict], out_path: str):
    if not accuracy_metrics:
        _plot_empty(out_path, "Comparison unavailable", "Train a model to compare ML output with regex rules.")
        return

    comparison = accuracy_metrics.get("per_type_comparison") or {}
    if not comparison:
    ...
    ax.set_title("ML vs Regex Comparison", fontsize=13, fontweight="bold")
